...
app.post('/showComments', (req, res, next) => {
    posts.find({ _id: new mongo.ObjectId(req.body.id) }, { comments: 1, _id: 0 }).toArray((err, theComments) => {
        if (err) {
            next(err);
        }
        res.render('comments', {
            comments: theComments[0].comments
        }, (err, html) => {
            io.emit('comments', { post: req.body.id, comments: html });
        });
        res.end('showing comments');
    });
});
